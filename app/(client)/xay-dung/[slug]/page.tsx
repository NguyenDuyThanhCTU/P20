import { ServiceItems, ServiceItems4 } from "@assets/item";
...
export const metadata: Metadata = {
  title: "Xây Dựng | Sửa Nhà Cần Thơ",
...
const NewsPage = async ({ params }: { params: { slug: string } }) => {
  ...
  const Topic = ServiceItems4.filter(
    (item: any) => item.value === params.slug
  )[0];
  return (
    <>
      ...
      <div className="p:col-auto d:col-span-5">
        <div className="font-LexendDeca font-extralight ">
          {Data.length > 0 ? (
            ...
            <h1 className="text-[28px] font-semibold">
              Không tìm thấy tin tức {Topic?.label}
            </h1>
          )}
